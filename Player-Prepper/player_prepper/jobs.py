...
Scouting is the slow thing here.  Four hundred games off Chess.com is a dozen
HTTP round trips and a PGN parse each; filling engine suggestions for twenty
gaps is twenty Stockfish searches.  Either inside a request means a browser
that spins with no idea how far along it is, and a gateway timeout on the one
case that matters most -- the player with a lot of games, which is precisely
the player worth scouting.

So a scout is a job: its own thread, progress into a dictionary the browser
polls, and cancellable.  Cancellation is cooperative -- the worker is handed a
``should_stop`` and checks it between games -- because a half-finished fetch
should still leave the games it already has, and killing a thread mid-search
orphans a Stockfish process.
